...
class CacheUtility
{
	...
	public static function createTakonomyCache()
	{
		self::createCacheFile();
		...
		foreach($dataList as $row)
		{
			...
			$content .= '$cachedTakonomies[\'' . $row['id'] . '\']=[' . self::$newLine;
			...
			$content .= self::getCacheItem('name', $row);
			$content .= self::getCacheItem('alias', $row);
			$content .= self::getCacheItem('description', $row);
			$content .= self::getCacheItem('contents', $row, 'int');
			$content .= self::getCacheItem('sort_num', $row, 'int');
			$content .= self::getCacheItem('type', $row, 'int');
			
			$content .= "];" . self::$newLine;
		}
		
		self::writeFile('cachedTakonomies.php', $content);
	}
